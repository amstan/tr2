...
def _register_info(register):
	...
	reg_name=_registers[reg_id]
	reg_len=_register_lens[reg_id]
...
class Stepper(object):
	def __init__(self,spi):
		...
		try:
			...
			pass
		self.get_status() #clear the statuses
	
	def nop(self,count=5):
		for i in range(count):
			self.spi.xfer(_commands["NOP"])
	
	def __getitem__(self,reg):
		"""Gets register"""
		reg_id,reg_name,reg_len=_register_info(reg)
		
		self.spi.xfer(_commands["GET_PARAM"]+reg_id)
	...
	def __setitem__(self,reg,value):
		...
		self.spi.xfer(_commands["SET_PARAM"]+reg_id)
	...
	def run(self,speed,forward=True):
		...
		self.spi.xfer(_commands["RUN"]+direction)
		for b in split_bytes(speed,num_bytes=3):
			self.spi.xfer(b)
	
	def move(self,steps,forward=True):
		direction=int(bool(forward))
		self.spi.xfer(_commands["MOVE"]+direction)
		for b in split_bytes(steps,num_bytes=3):
			self.spi.xfer(b)
	
	def reset_pos(self):
		self.spi.xfer(_commands["RESET_POS"])
	def reset(self):
		self.spi.xfer(_commands["RESET_DEVICE"])
	def soft_stop(self):
		self.spi.xfer(_commands["SOFT_STOP"])
	def hard_stop(self):
		self.spi.xfer(_commands["HARD_STOP"])
	def soft_hiz(self):
		self.spi.xfer(_commands["SOFT_HIZ"])
	def hard_hiz(self):
		self.spi.xfer(_commands["HARD_HIZ"])
	...
	def get_status(self):
		...
		self.spi.xfer(_commands["GET_STATUS"])
	...
	def check_errors(self):
		...
		status=self.status #cache the status
